...
async function loadConfig(): Promise<LibPathConfig> {
  ...
  if (await fs.exists(configPath)) {
    config = (await import(path.toFileUrl(path.resolve(configPath)).toString()))
  ...
  return {
    SDL3: path.resolve(sdl3),
    SDL3_image: path.resolve(sdl3_image),
    SDL3_ttf: path.resolve(sdl3_ttf),
  };
}
...
function* defaultLibPaths(): Generator<string> {
  ...
  if (
    import.meta.dirname &&
    path.normalize(".") != path.normalize(path.join(import.meta.dirname, ".."))
  ) {
    yield* lib.map((dir) =>
      path.normalize(path.join(import.meta.dirname!, "..", dir))
    );
  }
}
